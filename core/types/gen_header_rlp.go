...
func (obj *Header) EncodeRLP(_w io.Writer) error {
	...
	if _tmp7 {
		if obj.BlockAccessList == nil {
			w.Write([]byte{0xC0})
		} else {
			_tmp8 := w.List()
			for _, _tmp9 := range *obj.BlockAccessList {
				_tmp10 := w.List()
				w.WriteBytes(_tmp9.Address[:])
				_tmp11 := w.List()
				for _, _tmp12 := range _tmp9.Slots {
					_tmp13 := w.List()
					w.WriteBytes(_tmp12.Slot[:])
					w.ListEnd(_tmp13)
				}
				w.ListEnd(_tmp11)
				w.ListEnd(_tmp10)
